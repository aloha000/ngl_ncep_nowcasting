#!/usr/bin/env python3
"""Convert NGL TRO archives to one 5-minute, restartable Zarr store."""
...
def parse_args():
    ...
    parser.add_argument("--output", type=Path, default=root / "dataset" / "ngl_5min.zarr")
...
def parse_epoch(epoch: str) -> np.datetime64:
    ...
    date = np.datetime64(f"{year:04d}-01-01T00:00:00", "s")
    return date + np.timedelta64(day - 1, "D") + np.timedelta64(seconds, "s")


def parse_station(task):
    station_id, station_dir, start_ns, end_ns, n_steps = task
    start = np.datetime64(start_ns, "ns")
    end = np.datetime64(end_ns, "ns")
    ztd = np.full(n_steps, np.nan, dtype=np.float32)
    zwd = np.full(n_steps, np.nan, dtype=np.float32)
    records = 0
    for year in range(pd.Timestamp(start_ns).year, pd.Timestamp(end_ns).year + 1):
        ...
        try:
            ...
            continue
    return station_id, ztd, zwd, records
...
def create_store(path: Path, station_ids, times, force: bool):
    ...
    for variable in ("ztd", "zwd"):
        array = group.create_dataset(
            ...
            chunks=(min(8760 * 12, n_time), 1),  # one year of 5-minute steps
            dtype="f4",
            fill_value=np.nan,
    ...
    group.attrs.update(
        {
            "title": "NGL 5-minute zenith tropospheric delays",
            "start_utc": np.datetime_as_string(times[0], unit="s"),
            "end_utc": np.datetime_as_string(times[-1], unit="s"),
            "time_sampling": "5 minutes; exact UTC 5-minute records only",
...
def main():
    ...
    times = pd.date_range(start, end, freq="5min").tz_localize(None).to_numpy(dtype="datetime64[ns]")
    ...
    total_records = int(group.attrs.get("n_records", 0))
    ...
    try:
        for done, (station, ztd, zwd, records) in enumerate(results, 1):
